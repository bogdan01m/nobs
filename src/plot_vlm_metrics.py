...
def plot_vlm_performance(
    ...
    output_path_ttft: Path = Path("results/vlm_ttft.png"),
    output_path_tps: Path = Path("results/vlm_tps.png"),
) -> None:
    """
    ...
    for result in results:
        ...
        if vlm_task and "model" in vlm_task:
            ...
            # Check if metrics exist (they might be None)
            if model_data.get("final_50p_ttft_s") and model_data.get(
    ...
    if not devices:
        print("No VLM results found to plot")
        return
    ...
    ax1.set_title(
        "VLM Inference Performance: Time To First Token\n(Lower is Better)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    print(f"✅ VLM TTFT plot saved to {output_path_ttft}")
    plt.close()
    ...
    ax2.set_title(
        "VLM Inference Performance: Throughput\n(Higher is Better)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    print(f"✅ VLM TPS plot saved to {output_path_tps}")
    plt.close()
